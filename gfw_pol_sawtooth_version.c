...
/*                    second facet,including phase correction psi             */
     ...
     res1=res2=res3=res4=0.0;
      ll[1]=el1; ul[1]=el;                  /* z limits for 2nd facet */
	
      alpha=alpha2;
      psi=2*pi*el1/el;
      dum= el*tan(alpha2);          /* dum accounts for diff. expressions for impact param. for the two facets */
     D= k*(1/beta-cos(theta))-kx*tan(alpha);
      temp1=temp2=temp3=temp4=0.0;
      minpts=1;

      d01wcc(ndim, QU1, ll, ul, &minpts, maxpts, eps, &res1, &acc,&COMMWC5, &fail);  /*THIS LINE BREAKS WITH ECHELLE (AND REVERSE ECHELLE???? because the limits are the same value - there is no integration to carry out)*/ 
	
      minpts=1;
      d01wcc(ndim, QU2, ll, ul, &minpts, maxpts, eps, &res2, &acc,&COMMWC6, &fail); /*THIS LINE BREAKS WITH ECHELLE (AND REVERSE ECHELLE????)*/ 
	             
     G2x=Ccreate((ff*res1*tan(alpha2)), (ff*res2*tan(alpha2)));       /*changed 25/2/10*/
      G2z=Ccreate((ff*res1), (ff*res2));                              /*changed 25/2/10*/
     
     temp3=temp4=0.0;
      minpts=1;
      d01wcc(ndim, QU3, ll, ul, &minpts, maxpts, eps, &res3, &acc,&COMMWC7, &fail); /*THIS LINE BREAKS WITH ECHELLE (AND REVERSE ECHELLE????)*/ 
     minpts=1;
      d01wcc(ndim, QU4, ll, ul, &minpts, maxpts, eps, &res4, &acc, &COMMWC8, &fail); /*THIS LINE BREAKS WITH ECHELLE (AND REVERSE ECHELLE????)*/ 
    G2y=Ccreate((ff*res3*tan(alpha2)), (-ff*res4*tan(alpha2)));      /*changed 25/2/10*/       
 /* combine the 2 facets...   */   
      Gx= Cadd(G1x, G2x);
      Gy= Cadd(G1y, G2y);
      Gz= Cadd(G1z, G2z);
/* and create the complex vector G    */
      G= Vcreate( Gx.r, Gx.i, Gy.r, Gy.i, Gz.r, Gz.i);
